...
test.describe('Navigation (NAV-01, NAV-04)', () => {
  test('nav is visible with all expected links', async ({ page }) => {
    ...
    // Desktop nav links (scoped to #main-nav to avoid mobile menu / footer duplicates)
    await expect(page.locator('#main-nav a[href="/pricing"]')).toBeVisible();
    await expect(page.locator('#main-nav a[href="/why-fierro"]')).toBeVisible();
  });

  test('Login link points to app.getfierro.com/login', async ({ page }) => {
    await page.goto('/');
    const loginLink = page.locator('#main-nav a[href="https://app.getfierro.com/login"]');
    await expect(loginLink).toBeVisible();
  });

  test('Start Free CTA points to app.getfierro.com/signup', async ({ page }) => {
    await page.goto('/');
    const ctaLink = page.locator('#main-nav a[href="https://app.getfierro.com/signup"]');
